...
void Test::createComponent(const std::string &name,
                           ...
                           VPLTime_t time)
{
    int rv = db->createComponentsToRootIfMissing(name, version, time);
    if (rv != DATASETDB_OK)
...
{
    int rv = db->TestAndCreateComponent(name, is_directory, version, time);
...
void Test::testSetComponentPath(const std::string &name,
                                ...
                                int expectedErrorCode)
{
    int rv = db->SetComponentPath(name, path, version, time);
    if (rv != expectedErrorCode)
	fatal("Unexpected error code %d, expected %d", rv, expectedErrorCode);
    if (rv == DATASETDB_OK)
...
void Test::testSetComponentVersion(const std::string &name, 
				   ...
				   int expectedErrorCode)
{
    int rv = db->SetComponentVersion(name, version, time);
    if (rv != expectedErrorCode)
	fatal("Unexpected error code %d, expected %d", rv, expectedErrorCode);
    if (rv == DATASETDB_OK)
...
void Test::testRestoreTrash(u64 trash_version, u32 index,
                            ...
                            int expectedErrorCode)
{
    int rv = db->RestoreTrash(trash_version, index, name, version, time);
    if (rv != expectedErrorCode)
	fatal("Unexpected error code %d, expected %d", rv, expectedErrorCode);
    if (rv == DATASETDB_OK) {
...
void Test::testMoveComponent(const std::string &old_name,
                             ...
                             int expectedErrorCode)
{
    int rv = db->MoveComponent(old_name, new_name, version, time);
    if (rv != expectedErrorCode)
	fatal("Unexpected error code %d, expected %d", rv, expectedErrorCode);
}
...
void Test::testCopyComponent(const std::string &old_name,
                             ...
                             int expectedErrorCode)
{
    int rv = db->CopyComponent(old_name, new_name, version, time);
    if (rv != expectedErrorCode)
	fatal("Unexpected error code %d, expected %d", rv, expectedErrorCode);
}
