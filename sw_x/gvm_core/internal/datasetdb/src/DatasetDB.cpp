...
DEFSQL(CREATE_COMPONENT,                                                \
       "INSERT INTO component (name, ctime, mtime, type, version) "     \
       "VALUES (:name, :time, :time, :type, :version)");
DEFSQL(CREATE_COMPONENT_NOCASE,                                         \
       "INSERT INTO component (name, name_upper, ctime, mtime, type, version) " \
       "VALUES (:name, :name_upper, :time, :time, :type, :version)");
...
DEFSQL(GET_COMPONENT_BY_NAME,                                           \
       "SELECT id, name, content_id, ctime, mtime, type, version, trash_id " \
       "FROM component "                                                \
       "WHERE name=:name AND trash_id IS NULL");
...
DEFSQL(GET_COMPONENT_BY_VERSION_INDEX_NAME,                             \
       "SELECT c.id, c.name, c.content_id, c.ctime, c.mtime, c.type, c.version, c.trash_id " \
...
DEFSQL(COPY_COMPONENTS_BY_NAME,                                         \
       "INSERT INTO component (name, content_id, ctime, mtime, type, version) " \
       "SELECT replaceprefix(name, :old, :new), content_id, ctime, mtime, type, :version " \
       "FROM component "                                                \
       "WHERE (name=:old OR name GLOB :old||'/*') AND trash_id IS NULL");
DEFSQL(COPY_COMPONENTS_BY_NAME_NOCASE,                                  \
       "INSERT INTO component (name, name_upper, content_id, ctime, mtime, type, version) " \
       "SELECT replaceprefix_nocase(name, :old_upper, :new), "                \
       "utf8_upper(replaceprefix_nocase(name_upper, :old_upper, :new)), "      \
       "content_id, ctime, mtime, type, :version "                      \
       "FROM component "                                                \
       "WHERE (name_upper=:old_upper OR name_upper GLOB :old_upper||'/*') AND trash_id IS NULL");
...
DatasetDBError DatasetDB::OpenDB(const std::string &dbpath, u32 options)
{
    ...
    // Use persistent journals (save on inode operations)
    rv = sqlite3_exec(db, "PRAGMA journal_mode=PERSIST", NULL, NULL, &errmsg);
    if (rv != SQLITE_OK) {
	LOG_ERROR("Failed to enable PERSIST journaling: %d(%d): %s", rv, sqlite3_extended_errcode(db), errmsg);
	sqlite3_free(errmsg);
        rv =  mapSqliteErrCode(rv);
        goto exit;
    }
						 ...
						 VPLTime_t time)
{
    ...
    if (rv == DATASETDB_OK) {
        ...
        rv = createComponentsIfMissing(names, version, time);
        CHECK_RV(rv, rc, end);
                                                   ...
                                                   u64 &component_id)
{
	...
	if (componentAttrs.type != type) {
        ...
        rv = createComponent(name, version, time, type, component_id);
        CHECK_RV(rv, rc, end);
						    ...
						    VPLTime_t time)
{
    ...
    for (it = names.begin(); it != names.end(); it++) {
        u64 component_id;
	int rv = createComponentIfMissing(it->first, version, time, it->second, component_id);
	CHECK_RV(rv, rc, end);
    }
