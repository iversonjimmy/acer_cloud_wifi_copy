...
void short2net(unsigned short int s, unsigned char **bufp)
{
    unsigned short int ns = 0;
    ns = MDDNet_Htons(s);
    **bufp = (unsigned char)ns;
    *(*bufp + 1) = (unsigned char)(ns >> 8);
    *bufp += 2;    
}

void long2net(unsigned long int l, unsigned char **bufp)
{
    unsigned long int nl = 0;
    nl = MDDNet_Htonl(l);
    **bufp = (unsigned char)nl;
    *(*bufp + 1) = (unsigned char)(nl >> 8);
    *(*bufp + 2) = (unsigned char)(nl >> 16);
    *(*bufp + 3) = (unsigned char)(nl >> 24);
    *bufp += 4;
}
...
static void _label(struct message *m, unsigned char **bufp, char **namep)
{
    ...
    *name = '\0';
    for (x = 0; x <= 19 && m->_labels[x]; x++) {
        if (strcmp(*namep, (char *)m->_labels[x]))
            continue;
        *namep = (char *)m->_labels[x];
        return;
    }
    // no cache, so cache it if room
    if (x <= 19 && m->_labels[x] == 0)
        m->_labels[x] = (unsigned char *)*namep;
    m->_len += (name - *namep) + 1;
}
...
static int _rrparse(struct message *m, struct resource *rr, int count, unsigned char **bufp)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        rr[i].class_ = net2short(bufp);
        rr[i].ttl = net2long(bufp);
        rr[i].rdlength = net2short(bufp);

        // if not going to overflow, make copy of source rdata
        if (((unsigned int)rr[i].rdlength + (unsigned int)(*bufp - m->_buf)) > m->_buf_len ||
                rr[i].rdlength + (*bufp - m->_buf) > MAX_PACKET_LEN ||
                m->_len + rr[i].rdlength > MAX_PACKET_LEN) {
            return 1;
        }
        ...
        case QTYPE_PTR:
            ...
            break;
        case QTYPE_TXT:
            rr[i].known.txt.txt_raw = rr[i].rdata;
            *bufp += rr[i].rdlength;
            break;
        case QTYPE_AAAA:
           if (rr[i].rdlength != 16) {
                fprintf(stderr, "Data size mis-match for AAAA, RR data size: %d\n", rr[i].rdlength);
                return 1;
            }
            rr[i].known.aaaa.ipv6 = rr[i].rdata;
            *bufp += rr[i].rdlength;
            break;
        case QTYPE_SRV:
...
void message_parse(struct message *m, unsigned char *packet, unsigned int packet_len)
{
    unsigned char *buf;
    int i;
    ...
    /*{
        ...
        while(m->_len&7) {
            m->_len++;
            m->qd = (void *)(m->_packet + m->_len);
            //(void*)(m->_packet + m->_len);
...
void message_qd(struct message *m, char *name, unsigned short int type, unsigned short int class_)
{
    m->qdcount++;
    if (m->_buf == 0)
    ...
    short2net(class_, &(m->_buf));
}

static void _rrappend(struct message *m, char *name, unsigned short int type, unsigned short int class_, unsigned long int ttl)
{
    if (m->_buf == 0)
        m->_buf = m->_packet + 12; // initialization
    _host(m, &(m->_buf), name);
...
void message_an(struct message *m, char *name, unsigned short int type, unsigned short int class_, unsigned long int ttl)
{
    m->ancount++;
    _rrappend(m, name, type, class_, ttl);
}

void message_ns(struct message *m, char *name, unsigned short int type, unsigned short int class_, unsigned long int ttl)
{
    m->nscount++;
    _rrappend(m, name, type, class_, ttl);
}

void message_ar(struct message *m, char *name, unsigned short int type, unsigned short int class_, unsigned long int ttl)
{
    m->arcount++;
    _rrappend(m, name, type, class_, ttl);
}
