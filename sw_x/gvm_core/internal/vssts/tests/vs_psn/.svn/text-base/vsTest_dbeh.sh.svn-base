...
usage()
{
    ...
    echo "OR"
    echo
    echo "$script [-L] CCD_APPDATA_PATH <full cmd and args to call vs_psn>"
    echo
    ...
    echo
    echo "  Everything after CCD_APPDATA_PATH is the cmd and args to call vs_psn. These "
    echo "  are the same cmd and arguments that you would normally use when running vs_psn."
    echo
    echo "  This script manipulates db files, stops and starts the CCD, and calls vs_psn multiple times."
    echo "  When it runs vs_psn it passes \"-b $CCD_APPDATA_PATH/cc/sn/<number>/<number>\""
...
coredumps=$(dirname $(cat /proc/sys/kernel/core_pattern))
log "$(L 2>&1)" "coredump directory is $coredumps"
...
# In addition to a couple of files that are used for explicit checks,
# an entire directory tree is created by $testApp -e $CreateFileExpectSuccess
#
# In addition to a couple of other files that are used for explicit checks,
# another directory tree is created by $testApp -e $CreateFile2ExpectSuccess
#
# The trees are refered to as sdb and ldb (small db tree and large db tree).
# Their base dirnames are /sdbtree and /ldbtree
#
# The trees are checked when various test enum values passed to the testApp
# but it is not aplicable to all test enum values.
#
# The sdbtree is tested similar to the explicit files created for small db
# and the ldbtree is tested similar to the files created for large db.
#
# The testing consists of traversing the trees and checking the directories
# and files exist and the files contain the expected content.
#
# Below is the list of test enum values and whether sdb and/or ldb are
# checked by the app and the expected result.
#
#    CreateFileExpectSuccess
#        sdb created and checked
#
#    ReadFileExpectSuccess
#        sdb ok on read
#
#    ReadFileExpectNotFound
#        This is available but is never used in the tests
#        na - only applies to /dbErrTestDir1/file1.txt
#
#    ExpectDBCorruptionOnOpen
#        na - will get error on VSSI_OpenObjectTS
#
#    ExpectDBIrrecoverablyFailed
#        na - will get error on VSSI_OpenObjectTS
#
#    CreateFile2ExpectSuccess
#        ldb created and checked
#
#    ReadDir2AndFile2ExpectSuccess
#        ldb ok on read
#
#    ReadDir2AndFile2ExpectNotFound
#        small db swapped in for large db
#        ldb base dir not found
#
#    ReadDir2AndFile2ExpectFileNotFound
#        big db swapped in for small db
#        ldb base dir present but files deleted by fsck
#        ldb traverse returns VSSI_NOTFOUND
#
#    ReadFileExpectSuccessButFileLarger
#        sdb ok on read
#
#    ReadFileExpectSuccessButFileSmaller
#        sdb ok on read
#
#    ReadFileExpectCCDStopped
#        sdb VSSI_COMM or VSSI_NOTFOUND
#
#    ExpectDBCorruptionOnfsck
#        sdb VSSI_COMM or VSSI_NOTFOUND
#
#    CheckDBBackupCreation
#        na checks create backup and exits
#
#    SetupForDBFragmentationTest
#        na - has its own dir tree
#
#    TestDBFragmentationEffects
#        na - has its own dir tree
...
db_lost_found="${db_dir}/lost-and-found-system"
db_data_0_0="$db_dir/data/0/0"
file1="$db_dir/data/0/0/x1"
zeroSizeFile1="$db_dir/data/0/0/x2"
...
#cannot expect this due to lost & found directory feature
...
setInitialConditions "$(L 2>&1)"  "Setup small db again (testName: $testName)"
...
testName=dbeh_delete_file

setInitialConditions "$(L 2>&1)"  "Test deleting a file (testName: $testName)"

[ -f $file1 ] || fail "$(L 2>&1)" "expected $file1 to exist"
echo "File $file1 exists as expected"

$testApp -e $DeleteFileExpectSuccess || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "file not deleted from db"
}

[ ! -f $file1 ] || fail "$(L 2>&1)" "$file1 still exists after deleting"
echo "File $file1 does not exist as expected"

log "$(L 2>&1)" "TC_RESULT = PASS ;;; TC_NAME = $testName"

testName=$testSuiteName

stopCCD "$(L 2>&1)" "Stop CCD after delete file test"
############################################################

# run test that tests with 4 errors

testName=dbeh_4_errors

setInitialConditions "$(L 2>&1)" "Set initial conditions for testing with 4 errors (testName: $testName)"

log "$(L 2>&1)" "Test when db has the 4 fsck errors (testName: $testName)"

log "$(L 2>&1)" "Content of $db_dir before before rm rf is:";  ls -lt "$db_dir/"

rm -rf $db_dir/*

log "$(L 2>&1)" "Content of $db_dir before copying test package is:";  ls -lt "$db_dir/"
#wget http://pcstore.ctbg.acer.com/pc/test_data/orbe_test_data/test_data.20131106.tar
tar xf test_data.20131106.tar -C $db_dir

log "$(L 2>&1)" "Content of $db_dir after copying test db is:";  ls -lt "$db_dir/"

fileNoContentEntry="$db_dir/data/0/0/xff"
noFile="$db_dir/data/0/0/xfe"
...
stopCCD "$(L 2>&1)" "Stop ccd"

[ -e "$db" ] || fail "$(L 2>&1)" "expect db to be present"
[ -e "$db_is_open" ] && fail "$(L 2>&1)"  "Did not expect db-is-open marker"

log "$(L 2>&1)" "Content of $db_dir after creating db_needs_fsck flag is:";  ls -lt "$db_dir/"
...
$testApp -e $DB4Errors || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "failed to finish fsck"
}

log "$(L 2>&1)" "Content of $db_dir after triggering fsck is:";  ls -lt "$db_dir/"

[ -e "$db_needs_fsck" ] && fail "$(L 2>&1)"  "Did not expect db-needs-fsck marker"
...
log "$(L 2>&1)" "Remove $db_dir"
rm -rf $db_dir/*
log "$(L 2>&1)" "Content of $db_dir is:";  ls -lt "$db_dir/"

startCCD "$(L 2>&1)" "Start CCD"
...
testName=dbeh_check_lost_found_dir

setInitialConditions "$(L 2>&1)" "Set initial conditions for testing of lost and found directory"
[ -e "$db_lost_found" ] || fail "$(L 2>&1)"  "$db_lost_found does not exist."
    
log "$(L 2>&1)" "Run test program that checks for 'Lost & Found' directory (testName: $testName)"
$testApp -e $CheckLostFoundDir || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "failed check for 'Lost & Found' directory"
}
log "$(L 2>&1)" "TC_RESULT = PASS ;;; TC_NAME = $testName"
    
testName=dbeh_check_lost_found_system
log "$(L 2>&1)" "Testing of user-created 'Lost & Found' directory. (testName: $testName)"
stopCCD "$(L 2>&1)" "Stop ccd"
log "$(L 2>&1)" "Forcibly remove $db_lost_found flag"
rm -f "$db_lost_found"
startCCD "$(L 2>&1)" "Start ccd"
[ -e "$db_lost_found" ] && fail "$(L 2>&1)" "Did not expect $db_lost_found marker"
$testApp -e $CheckLostFoundSystem || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "failed check for system 'Lost & Found' directory"
}
...
# run test that tests unreferenced files

testName=dbeh_lost_found_unreferenced

setInitialConditions "$(L 2>&1)" "Set initial conditions for testing with unreferenced files (testName: $testName)"
...
$testApp -e $CheckLostFoundUnref || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "failed check for unreferenced files"
}
log "$(L 2>&1)" "TC_RESULT = PASS ;;; TC_NAME = $testName"
    
############################################################

# run test that tests orphaned files

testName=dbeh_lost_found_orphaned

setInitialConditions "$(L 2>&1)" "Set initial conditions for testing with orphaned entries (testName: $testName)"
...
log "$(L 2>&1)" "Content of $db_dir before copying test db is:";  ls -lt "$db_dir/"

#wget http://pcstore.ctbg.acer.com/pc/test_data/orbe_test_data/test_data.20131118.tar
tar xf test_data.20131118.tar -C $db_dir
...
$testApp -e $CheckLostFoundOrphaned || {
    rv=$?
    testAppError "$(L 2>&1)" $rv "failed check for orphaned entries"
}
log "$(L 2>&1)" "TC_RESULT = PASS ;;; TC_NAME = $testName"
