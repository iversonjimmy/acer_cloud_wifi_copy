...
#define TEST_DB_UNREF_FILES 189
#define TEST_DB_ORPHANED_ENTRIES 2
#define TEST_DB_ORPHANED_ENTRIES_2 5
...
static int create_dir_tree(vscs_test_context_t& test_context, VSSI_Object handle, const char *dirname, int num_levels, int num_dirs, int num_files, size_t fileBaseSize = 0);
static int traverse_dir_tree(vscs_test_context_t& test_context, VSSI_Object handle, char *dirname, int num_levels, int num_dirs, int num_files, bool isdelete, size_t fileBaseSize = 0);
static int time_to_traverse(vscs_test_context_t& test_context, VSSI_Object handle, const char *dirname_base, const int num_top_level_trees, const int num_subtrees, int num_levels, int num_dirs, int num_files, size_t fileBaseSize, bool isMissingTopLevelOK);
static const int DBEH_TTT_TREE_NOTFOUND  = -237;
static const int DBEH_TDT_SDIR_NOTFOUND  = -236;
static const int DBEH_TDT_FILE_NOTFOUND  = -235;
static const int DBEH_TDT_FILE_STAT_SIZE = -234;
static const int DBEH_TDT_FILE_READ_SIZE = -233;
static const int DBEH_TDT_FILE_CONTENT   = -232;
...
static int test_vscs_access_save_bvd_async(u64 user_id, u64 dataset_id)
{
    int rv = 0;
    int rc;
    VSSI_Object handle;
...
static int test_vss_file_handles( u64 user_id,
                                  u64 dataset_id,
...
static int test_vss_file_rename_remove(u64 user_id,
                                       u64 dataset_id)
{
    int rv = 0;
    int rc;
    ...
    VSSI_Rename2(handle, name1, name1, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
    ...
    // This should fail
    VSSI_Rename2(handle, dirname1, dirname2, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_BUSY) {
    ...
    // Rename it back
    VSSI_Rename2(handle, dirname2, dirname1, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
    ...
    VSSI_Rename2(handle, name1, name4, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
    ...
    // Rename it back
    VSSI_Rename2(handle, name4, name1, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
...
#define CHUNK_SIZE    (1024 * 1024)
    ...
    VSSI_Rename2(handle, nameUTF8_Aa, nameUTF8_ab, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
...
static int test_vss_file_remove_shared(
                                       u64 user_id,
                                       u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_rename_attributes(
                                       u64 user_id,
                                       u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_create(
                                u64 user_id,
                                u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_modify_time(
                                u64 user_id,
                                u64 dataset_id)
{
    int rv = 0;
...
// Test that the poll thread will be recreated after a VSSI_Cleanup.
static const char vsTest_vss_poll_thead_test[] = "VSSI Polling Thread Test";
static int test_vss_poll_thread(
                                u64 user_id,
                                u64 dataset_id)
{
    int rv = 0;
    ...
    // For VSSI_Init, 11 to match the Init in vsTest_psn.cpp
    u64 app_id = 11;
    ...
    vsTest_curTestName = vsTest_vss_poll_thead_test;
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0,
                        "Starting test: %s.",
                        vsTest_curTestName);
    ...
    // Cleanup and reinitalize a few times.

    VPLTRACE_LOG_INFO(TRACE_APP, 0, "Cleanup and initialize VSSI once.");
    VSSI_Cleanup();
    rc = VSSI_Init(app_id);
    if( rc != VSSI_SUCCESS) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "VSSI_Init failed with: %d", rv);
        rv++;
        goto done;
    }

    VPLTRACE_LOG_INFO(TRACE_APP, 0, "Cleanup and initialize VSSI twice.");
    VSSI_Cleanup();
    rc = VSSI_Init(app_id);
    if( rc != VSSI_SUCCESS) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "VSSI_Init failed with: %d", rv);
    ...
    // Do an operation that requires us to send a command. 
    // If the polling htread is active, it will pick it up and handle it.
    // If it is not active, we'll never hear back and timeout.

    VPLTRACE_LOG_INFO(TRACE_APP, 0, "Trying to send open object command and receive reply...");
    VSSI_OpenObjectTS(user_id, dataset_id, 
                         VSSI_READWRITE | VSSI_FORCE, &handle,
                         &test_context, vscs_test_callback);
    rc = VPLSem_TimedWait(&(test_context.sem), VPLTime_FromSec(30));
    if (rc == VPL_ERR_TIMEOUT) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                        "VSSI_OpenObjectTS timed out %d", 
                        rc);
        rv++;
        goto done;
    }
                          ...
                          u32 attrs, u32 attrs_mask)
{
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "BEFORE VSSI_ChmodFile");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_ChmodFile");
done:
    return rc;
}
...
static int vss_remove(vscs_test_context_t& test_context, VSSI_Object handle, const char* name, bool failIfNotFound)
{
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Before VSSI_Remove");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_Remove");
...
// fileBaseSize should be small (e.g. 256) because file content buffer is allocated on stack
static int create_dir_tree(vscs_test_context_t &test_context, VSSI_Object handle, const char *dirname, int num_levels, int num_dirs, int num_files, size_t fileBaseSize)
{
    int rc = 0;
    char filename[50];
    char subdirname[50];
...
//                         "VSSI_OpenFile returns %d.", rc);
            ...
            if (fileBaseSize) {
                const size_t myBufSize = fileBaseSize + sizeof filename;
                char myBuf[myBufSize];
                u32 wrLen;

                for (int i = 0; i < fileBaseSize; ++i) {
                    myBuf[i] = i;
                }
                memcpy (&myBuf[fileBaseSize], filename, strlen(filename) + 1);
                wrLen = fileBaseSize + strlen(filename) + 1;
...
//                         "VSSI_MkDir2 returns %d.", rc);
            rc = create_dir_tree(test_context, handle, subdirname, num_levels - 1,
                                  num_dirs, num_files, fileBaseSize);
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
...
static int traverse_dir_tree(vscs_test_context_t &test_context, VSSI_Object handle, char *dirname, int num_levels, int num_dirs, int num_files, bool isdelete, size_t fileBaseSize)
{
    int rc = 0;
    VSSI_Dir2 dir1 = NULL;
...
//                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Found dir :%s", subdirname);
                rc = traverse_dir_tree(test_context, handle, subdirname, num_levels-1,
                                        num_dirs, num_files, isdelete, fileBaseSize);
                if(rc != 0) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "traverse_dir %s returns %d.", subdirname, rc);
                    goto exit;
                }
                ...
                file_count++;
                char (&name)[sizeof subdirname] = subdirname;
                memcpy (name, dirname, strlen(dirname));
                sprintf (name+strlen(dirname), "%s%s%s", slash, dirent->name, nc);
//                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Found file :%s", name);
                ...
                if (isdelete) {
                ...
                else if (fileBaseSize) {
                    const size_t myReadBufSize = fileBaseSize + sizeof name;
                    char myReadBuf[myReadBufSize];
                    VSSI_Dirent2* stats = NULL;
                    ...
                    expectedStatSize = expectedReadLen = fileBaseSize + strlen(name) + 1;
                    ...
                    rdLen = myReadBufSize;
                    VSSI_ReadFile(fileHandle, 0, &rdLen, myReadBuf,
                                        &test_context, vscs_test_callback);
                    VPLSem_Wait(&(test_context.sem));
                    rc = test_context.rv;
                    if(rc != VSSI_SUCCESS) {
                        ...
                        rc = DBEH_TDT_FILE_READ_SIZE;
                    }
                    else {
                        for (i = 0; i < fileBaseSize; ++i) {
                            if ((u8)myReadBuf[i] != i % 256) {
                                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                                                "VSSI_ReadFile %s myReadBuf[%u] expected 0x%02x got 0x%02x.",
                                                name, i, (i % 256), (u8)myReadBuf[i]);
                                rc = DBEH_TDT_FILE_CONTENT;
                                break;
                            }
                        }
                        if (i == fileBaseSize) {
                            if ( strcmp (&myReadBuf[fileBaseSize], name) != 0) {
                                myReadBuf[fileBaseSize + strlen(name)] = 0;
                                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                                                "VSSI_ReadFile %s myReadBuf[%u] expected %s got %s.",
                                                name, fileBaseSize, name, &myReadBuf[fileBaseSize]);
                                rc = DBEH_TDT_FILE_CONTENT;
                            }
                            else if (false) {
                                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0,
                                       "Successfully read and checked %u bytes of %s",
                                       expectedReadLen, name);
                            }
                        }
                    }
...
static int time_to_traverse(vscs_test_context_t& test_context, VSSI_Object handle, const char *dirname_base, const int num_top_level_trees, const int num_subtrees, int num_levels, int num_dirs, int num_files, size_t fileBaseSize, bool isMissingTopLevelOK)
{
    ...
    for (m = 0; m < num_top_level_trees && !rc; ++m) {
        for (n = 0; n < num_subtrees && !rc; ++n) {

            memcpy (dirname, dirname_base, strlen(dirname_base));
            sprintf (dirname+strlen(dirname_base), "%d%s%d%s", m, us, n, nc);
            if ( num_top_level_trees*num_subtrees <= 10) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Traverse dir tree %s", dirname);
            }

            rc = vss_isFileOrDir(test_context, handle, dirname, &isDir, 0);
            if (rc == VSSI_SUCCESS) {
                rc = traverse_dir_tree(test_context, handle, dirname,
                                        num_levels, num_dirs, num_files, false, fileBaseSize);
                if(rc != 0) {
                    // errors may be expected, so isn't necessarily FAIL
                    VPLTRACE_LOG_ERR(TRACE_APP, 0,
                                 "Error traversing directory tree (%d %d) %s", m, n, dirname);
                }
            }
            else if (rc == VSSI_NOTFOUND) {
                const char* isItOkText;
                if (isMissingTopLevelOK) {
                    rc = 0;
                    isItOkText = " but isMissingTopLevelOK is true.";
                } else {
                    rc = DBEH_TTT_TREE_NOTFOUND;
                    isItOkText = ": DBEH_TTT_TREE_NOTFOUND";
                }
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is missing%s", dirname, isItOkText);
            }
        }
    }
...
static int test_vss_file_attributes (
                                     u64 user_id,
                                     u64 dataset_id)
{
    int rv = 0;
    ...
    const char* dirname1 = "/myDir1";
    const char* dirname2 = "/myDir1/myDir2";
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VPLSem_Init");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_Delete");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_OpenObjectTS");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER vss_remove");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_OpenFile");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER vss_attrs_check");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER chmod_file");
    // verify it's gone
    rc = vss_attrs_check(test_context, handle, name, attrs);
    if(rc != VSSI_SUCCESS) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_WriteFile");
    // Verify the Archive bit is set
    attrs |= VSSI_ATTR_ARCHIVE;
    rc = vss_attrs_check(test_context, handle, name, attrs);
    if(rc != VSSI_SUCCESS) {
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_CloseFile");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_ReadFile");
    ...
    VSSI_OpenFile(handle, name, flags, 0, &fileHandle1,
                  &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc == VSSI_SUCCESS) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "2nd and a half VSSI_OpenFile %s returns %d.", name, rc);
        rv++;
    }
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER attrs_check");
    ...
    VSSI_Rename2(handle, name2, name3, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
    ...
    attrs1 &= ~VSSI_ATTR_READONLY;
    rc = vss_chmod(test_context, handle, dirname1, attrs1, VSSI_ATTR_READONLY);

    if(rc != VSSI_SUCCESS) {
        VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "Chmod on directory - %d", rc);
        rv++;
    }
     // verify the attr
    rc = vss_attrs_check(test_context, handle, dirname1, attrs1);
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "AFTER VSSI_Chmod on directory");
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Before fail_open");
...
static int test_vss_mkdir_parents_and_existing (
                                     u64 user_id,
                                     u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_case_insensitivity (
                                     u64 user_id,
                                     u64 dataset_id)
{
    int rv = 0;
    ...
    VSSI_Rename2(handle, file1FullPathCases[1], file2FullPathCases[0],
                          VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
    if(rc != VSSI_SUCCESS) {
    ...
    VSSI_Rename2(handle, dir3FullPathCases[1], dir4FullPathCases[0],
                          VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
    VPLSem_Wait(&(test_context.sem));
    rc = test_context.rv;
...
static int test_vss_file_access_and_share_modes (
                                     u64 user_id,
                                     u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_open_many(
                                u64 user_id,
                                u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_file_oplocks(
                                  u64 user_id,
                                  u64 dataset_id)
{
    int rv = 0;
    ...
    // This test case is problematic.  It only works if SetByteRangeLock() is processed
    // by the server before the subsequent GetFileLockState().  There's no guarantee
    // of order for commands issued by different objects, which could use different
    // tunnels.  As a work-around, put in a 3-second sleep to give the SetByteRangeLock()
    // call a head-start.  We may need to put in a better solution later.
    sleep(3);
...
static int test_vss_file_oplocks_brlocks(
                                  u64 user_id,
                                  u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_object_teardown(
                                    u64 user_id,
                                    u64 dataset_id)
{
    int rv = 0;
...
static int test_vss_bad_objhandle(
                                  u64 user_id,
                                  u64 dataset_id)
{
    int rv = 0;
                           ...
                           const std::string& db_dir)
{
    int rv = 0;
    int rc;
    int rc2 = 0; /* Return value for 4 errors test */
    int rc0 = 0; /* Return value for 4 errors test */
    VSSI_Object handle;
    vscs_test_context_t test_context;
    static const char* lostFoundUser = "lost+found-(user)-";
    static const char* lostFoundDir = "/lost+found";
    static const char* dirnameIn = "/lost+found/ShouldNotBeAllowed";
    static const char* lostFoundRename = "/Lost & Found1";
    ...
    static const char* nameLF = "/lost+found/file1.txt";
    static const char* nameMissing1 = "/sdbtree0_1/dir1/dir0/dir0/dir0/file2"; /* No content entry file 0/0/xff */
    static const char* nameMissingFile0 = "/dbErrTestDir1/file0"; /* Component and content entries present but no file */
    static const char* name2 = "/dbErrTestDir1/dbfsckTestDir2/file2.txt";
    static char lostFoundMMDDYYYYFullPath[50];
    static char dirnameInMMDDYYYY[100];
    static char unrefFilesNameFullPath[100];
    static char unrefOneFileNameFullPath[150];
    const char* lostFoundMMDDYYYY = NULL;
    const char* unrefFilesName = NULL;
    const char* unrefFileName = NULL;
    static char orphanedEntriesNameFullPath[100];
    static char orphanedEntriesDirNameFullPath[150];
    const char* orphanedEntriesName = NULL;
    const char* orphanedEntryName = NULL;
    const char* nameMissingFile = NULL;
    const char* nameMissing = NULL;
    const char* name = NULL;
    const char* nameLostFound = NULL;
    ...
    const char* dirnameLostFound = NULL;
    bool found = false;
    bool foundMMDDYYYY = false;
    bool foundUnrefFiles = false;
    bool foundOrphanedDirEntry = false;
    bool foundOrphanedEntriesDir = false;
    u32 unrefFileCount = 0;
    u32 orphanedEntriesCount = 0;
    u32 flags;
    u32 attrs;
    VSSI_File fileHandle0 = NULL;
    VSSI_File fileHandle1 = NULL;
    VSSI_File fileHandle2 = NULL;
    VSSI_Dir2 dir = NULL;
    VSSI_Dirent2* dirent = NULL;
    ...
    const size_t frag_fileBaseSize = 0;

    // Constants for extra dbeh/fsck small and large db directory trees
    const int num_sdb_top_level_trees = 1;
    const int num_sdb_subtrees = 2;
    const int num_sdb_subtree_levels = 5;
    const int num_sdb_subtree_subdirs = 2;
    const int num_sdb_subtree_files = 3;
    static const char* sdb_dirname_base = "/sdbtree";
    const size_t sdb_fileBaseSize = 255;

    const int num_ldb_top_level_trees = 1;
    const int num_ldb_subtrees = 2;
    const int num_ldb_subtree_levels = 5;
    const int num_ldb_subtree_subdirs = 2;
    const int num_ldb_subtree_files = 3;
    static const char* ldb_dirname_base = "/ldbtree";
    const size_t ldb_fileBaseSize = 257;
    ...
    enum { CreateFileExpectSuccess = 1,
           ...
           DeleteFileExpectSuccess = 17,
           DB4Errors = 18,
           CheckLostFoundDir = 19,
           CheckLostFoundSystem = 20,
           CheckLostFoundUnref = 21,
           CheckLostFoundOrphaned = 22
         };
    ...
    // Sleep to avoid a test only race condition that has
    // only been an issue in long version of tests in rc branch.
    ...
    switch (db_err_hand_param) {
        ...
        case DB4Errors:
            nameMissingFile = nameMissingFile0;
            nameMissing = nameMissing1;
            name = name1;
            dirname = dirname1;
            zeroSizeFileName = zeroSizeFileName1;
            break;
        ...
        case CheckLostFoundDir:
        case CheckLostFoundSystem:
        case CheckLostFoundUnref:
        case CheckLostFoundOrphaned:
            dirnameLostFound = lostFoundDir;
            nameLostFound = nameLF;
            nameMissingFile = nameMissingFile0;
            nameMissing = nameMissing1;
            name = name1;
            dirname = dirname1;
    ...
    if ( db_err_hand_param == CreateFileExpectSuccess ||
            ...
            db_err_hand_param == CheckLostFoundDir ) {
        //
    ...
    if ( db_err_hand_param == DeleteFileExpectSuccess ) {
    ...
    // create more directories and files to check expected status for small db
    if ( db_err_hand_param == CreateFileExpectSuccess ) {
        ...
        for (m = 0; m < num_sdb_top_level_trees; ++m) {
            for (n = 0; n < num_sdb_subtrees; ++n) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb: m = %d n = %d", m, n);

                memcpy (dirname, sdb_dirname_base, strlen(sdb_dirname_base));
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb: dirname base is :"
                             "%s", dirname);

                sprintf (dirname+strlen(sdb_dirname_base), "%d%s%d%s", m, us, n, nc);
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb dirname is :"
                             "%s", dirname);
                ...
                rc = create_dir_tree(test_context, handle, dirname, num_sdb_subtree_levels,
                            num_sdb_subtree_subdirs, num_sdb_subtree_files, sdb_fileBaseSize);
                if(rc != 0) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
    ...
    if ( db_err_hand_param == CreateFile2ExpectSuccess ) {
        ...
        for (m = 0; m < num_ldb_top_level_trees; ++m) {
            for (n = 0; n < num_ldb_subtrees; ++n) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb: m = %d n = %d", m, n);

                memcpy (dirname, ldb_dirname_base, strlen(ldb_dirname_base));
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb: dirname base is :"
                             "%s", dirname);

                sprintf (dirname+strlen(ldb_dirname_base), "%d%s%d%s", m, us, n, nc);
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "sdb dirname is :"
                             "%s", dirname);
                ...
                // Create a directory tree[m,n]
                rc = create_dir_tree(test_context, handle, dirname, num_ldb_subtree_levels,
                            num_ldb_subtree_subdirs, num_ldb_subtree_files, ldb_fileBaseSize);
                if(rc != 0) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                                     "Error creating directory tree (%d %d) %s", m, n, dirname);
                    rv++;
                    goto exit;
                }
            }
    ...
    // Open "Lost & Found" directory to check it
    if ( (db_err_hand_param == CheckLostFoundDir) || (db_err_hand_param == CheckLostFoundSystem) ) {
        VSSI_OpenDir2(handle, dirnameLostFound, &dir,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc != VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                "VSSI_OpenDir2 %s for 'Lost & Found' dir returns %d.", dirnameLostFound, rc);
            rv++;
            goto exit;
        }
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "VSSI_OpenDir2 %s for 'Lost & Found' dir returns %d. as expected", dirnameLostFound, rc);
        VSSI_CloseDir2(dir);
        dir = NULL;

        // Try to delete 'Lost & Found' directory - should not be allowed                    
        VSSI_Remove(handle, dirnameLostFound,
            &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                "VSSI_Remove should not succeed for 'Lost & Found' folder %s: %d.", dirnameLostFound, rc);
            rv++;
            goto exit;
        }
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to remove %s as expected: %d.", dirnameLostFound, rc);
        
        // Try to rename 'Lost & Found' directory - should not be allowed                    
        VSSI_Rename2(handle, dirnameLostFound, lostFoundRename, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "Rename component %s to %s: %d.",
                         dirnameLostFound, lostFoundRename, rc);
            rv++;
            goto exit;
        }

        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to rename %s as expected: %d.", dirnameLostFound, rc);
        
        // Try to create a directory in 'Lost & Found' directory - should not be allowed
        VSSI_MkDir2(handle, dirnameIn, attrs,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:VSSI_MkDir2 %s returns %d.", dirnameIn, rc);
            rv++;
            goto exit;
        }
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to create directory %s as expected: %d.", dirnameIn, rc);

        // Try to rename a directory from outside 'Lost & Found' directory to inside 'Lost & Found' directory - should not be allowed                    
        VSSI_Rename2(handle, dirname, dirnameIn, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "Rename component %s to %s: %d.",
                         dirname, dirnameIn, rc);
            rv++;
            goto exit;
        }

        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to rename directory %s to %s as expected: %d.", dirname, dirnameIn, rc);

        // Check that 'Lost & Found-(user)-<timestamp>' directory is created for previously existing user data
        VSSI_OpenDir2(handle, dirnameLostFound, &dir,
         &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        ...
        while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
            if ( db_err_hand_param == CheckLostFoundSystem ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Comparing with %s", lostFoundUser);
                if (strncmp(dirent->name, lostFoundUser, sizeof lostFoundUser) == 0) {
                    found = true;
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", lostFoundUser);
                }
            }
        } 
        if ( db_err_hand_param == CheckLostFoundSystem ) {
            if (!found) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                            "%s not found using VSSI_ReadDir2.", lostFoundUser);
                rv++;
            }
        }
        if (dir) {
    ...
    // Check unreferenced file in "lost+found" directory
    if ( (db_err_hand_param == CheckLostFoundUnref)  || (db_err_hand_param == CheckLostFoundOrphaned) ) {
        VSSI_OpenDir2(handle, dirnameLostFound, &dir,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        ...
        while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
           VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
            if (strlen(dirent->name) >= 8) {
                // Search for MM-DD-YYYY directory - Use the "20" substring for year
                if (strncmp(&dirent->name[6], "20", 2) == 0) {
                    foundMMDDYYYY = true;
                    lostFoundMMDDYYYY = dirent->name;
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", lostFoundMMDDYYYY);
                    break;
                }
            }
        } 
        if (foundMMDDYYYY == false) {
            rv++;
            goto exit;
        }
        i = snprintf (lostFoundMMDDYYYYFullPath, sizeof(lostFoundMMDDYYYYFullPath), "%s%s%s", lostFoundDir, "/", lostFoundMMDDYYYY);
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is lostFoundMMDDYYYYFullPath. return value = %d", lostFoundMMDDYYYYFullPath, i);
        // OpenDir2 has succeeded so need to close dir
        VSSI_CloseDir2(dir);
        dir = NULL;

        i = snprintf (dirnameInMMDDYYYY, sizeof(dirnameInMMDDYYYY), "%s%s%s", lostFoundMMDDYYYYFullPath, "/", "someName");
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is dirnameInMMDDYYYY. return value = %d", dirnameInMMDDYYYY, i);
        // Try to create a directory in 'Lost & Found/MM-DD-YYYY' directory - should not be allowed
        VSSI_MkDir2(handle, dirnameInMMDDYYYY, attrs,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:VSSI_MkDir2 %s returns %d.", dirnameInMMDDYYYY, rc);
            rv++;
            goto exit;
        }
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to create directory %s as expected: %d.", dirnameInMMDDYYYY, rc);

        // Try to rename a directory from outside 'Lost & Found/MMDDYYYY' directory to inside 'Lost & Found' directory - should not be allowed                    
        VSSI_Rename2(handle, dirname, dirnameInMMDDYYYY, VSSI_RENAME_REPLACE_EXISTING,
                          &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "Rename component %s to %s: %d.",
                         dirname, dirnameInMMDDYYYY, rc);
            rv++;
            goto exit;
        }

        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
            "Not able to rename directory %s to %s as expected: %d.", dirname, dirnameInMMDDYYYY, rc);

        // Open MM-DD-YYYY directory
        VSSI_OpenDir2(handle, lostFoundMMDDYYYYFullPath, &dir,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        rc = test_context.rv;
        if(rc != VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
               "VSSI_OpenDir2 %s returns %d.", lostFoundMMDDYYYYFullPath, rc);
            rv++;
            goto exit;
        }
        if (db_err_hand_param == CheckLostFoundUnref) {
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
                if (strncmp(dirent->name,  "unreferenced-files-", strlen("unreferenced-files-")) == 0) {
                    foundUnrefFiles = true;
                    unrefFilesName = dirent->name;
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", unrefFilesName);
                    break;
                }
            } 
            if (foundUnrefFiles == false) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, " unreferenced-files-xxx NOT found using VSSI_ReadDir2.");
                rv++;
                goto exit;
            }
            i = snprintf (unrefFilesNameFullPath, sizeof(unrefFilesNameFullPath), "%s%s%s", lostFoundMMDDYYYYFullPath, "/", unrefFilesName);
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is unrefFilesNameFullPath. return value = %d", unrefFilesNameFullPath, i);
            // OpenDir2 has succeeded so need to close dir
            VSSI_CloseDir2(dir);
            dir = NULL;

            // Open unreferenced-files-<timestamp>  directory
            VSSI_OpenDir2(handle, unrefFilesNameFullPath, &dir,
                          &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc = test_context.rv;
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                   "VSSI_OpenDir2 %s returns %d.", unrefFilesNameFullPath, rc);
                rv++;
                goto exit;
            }
            unrefFileCount = 0;
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
                unrefFileCount++;
                unrefFileName = dirent->name;
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", unrefFileName);
            } 
                    
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d unreferenced files found in %s.", unrefFileCount, unrefFilesNameFullPath);
            if (unrefFileCount != TEST_DB_UNREF_FILES ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d unreferenced files found in %s.i Expected %d", unrefFileCount, unrefFilesNameFullPath, TEST_DB_UNREF_FILES);
                rv++;
            }
        
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "CloseDir and Open again.");
            // OpenDir2 has succeeded so need to close dir
            VSSI_CloseDir2(dir);
            dir = NULL;
            ...
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "OpenDir %s again to delete files.", unrefFilesNameFullPath);
            // Delete the files 1 by 1 - should succeed
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                unrefFileName = dirent->name;
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", unrefFileName);
                i = snprintf (unrefOneFileNameFullPath, sizeof(unrefOneFileNameFullPath), "%s%s%s", unrefFilesNameFullPath, "/", unrefFileName);
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is unrefFilesNameFullPath. return value = %d", unrefFilesNameFullPath, i);
                // Try to delete 'Lost & Found/MM-DD-YYYY' directory - should be allowed                    
                VSSI_Remove(handle, unrefOneFileNameFullPath,
                    &test_context, vscs_test_callback);
                VPLSem_Wait(&(test_context.sem));
                rc = test_context.rv;
                if(rc != VSSI_SUCCESS) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                    "VSSI_Remove should succeed for %s: %d.", unrefOneFileNameFullPath, rc);
                    rv++;
                    goto exit;
                } else {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
                    "Able to remove %s as expected: %d.", unrefOneFileNameFullPath, rc);
                }
            } 
            ...
            // Try to delete 'Lost & Found/MM-DD-YYYY/unreferenced-files-<timestamp>' directory - should be allowed                    
            VSSI_Remove(handle, unrefFilesNameFullPath,
                &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc = test_context.rv;
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                "VSSI_Remove should succeed for %s: %d.", unrefFilesNameFullPath, rc);
                rv++;
            } else {
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
                "Able to remove %s as expected: %d.", unrefFilesNameFullPath, rc);
            }
            // Try to delete 'Lost & Found/MM-DD-YYYY' directory - should be allowed                    
            VSSI_Remove(handle, lostFoundMMDDYYYYFullPath,
                &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc = test_context.rv;
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                "VSSI_Remove should succeed for 'Lost & Found/MM-DD-YYYY' folder %s: %d.", lostFoundMMDDYYYYFullPath, rc);
                rv++;
            } else {
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, 
                "Able to remove %s as expected: %d.", lostFoundMMDDYYYYFullPath, rc);
            }
            goto exit; 

        } else if (db_err_hand_param == CheckLostFoundOrphaned) {
        // check for parent being file type - expected 2
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
                if (strncmp(dirent->name,  "orphaned-entries-", strlen("orphaned-entries-")) == 0) {
                    foundOrphanedEntriesDir = true;
                    orphanedEntriesName = dirent->name;
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", orphanedEntriesName);
                    break;
                }
            } 
            if (foundOrphanedEntriesDir == false) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "orphaned-entries-xxx NOT found using VSSI_ReadDir2.");
                rv++;
                goto exit;
            }
            i = snprintf (orphanedEntriesNameFullPath, sizeof(orphanedEntriesNameFullPath), "%s%s%s", lostFoundMMDDYYYYFullPath, "/", orphanedEntriesName);
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is orphanedEntriesNameFullPath. return value = %d", orphanedEntriesNameFullPath, i);
            // OpenDir2 has succeeded so need to close dir
            VSSI_CloseDir2(dir);
            dir = NULL;

            // Open orphaned-entries-<timestamp>  directory
            VSSI_OpenDir2(handle, orphanedEntriesNameFullPath, &dir,
                          &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc = test_context.rv;
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                   "VSSI_OpenDir2 %s returns %d.", orphanedEntriesNameFullPath, rc);
                rv++;
                goto exit;
            }
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
                orphanedEntryName = dirent->name;
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", orphanedEntryName);
                if(dirent->isDir == true) {
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is a directory", orphanedEntryName);
                    foundOrphanedDirEntry = true;
                    break;
                } else {
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is a file", orphanedEntryName);
                }
            } 
                    
            if (foundOrphanedDirEntry == false) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "orphaned entry directory NOT found using VSSI_ReadDir2.");
                rv++;
                goto exit;
            }

            i = snprintf (orphanedEntriesDirNameFullPath, sizeof(orphanedEntriesDirNameFullPath), "%s%s%s", orphanedEntriesNameFullPath, "/", orphanedEntryName);
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is orphanedEntriesNameDirFullPath. return value = %d", orphanedEntriesDirNameFullPath, i);
            // OpenDir2 has succeeded so need to close dir
            VSSI_CloseDir2(dir);
            dir = NULL;

            // Open orphaned-entries-<timestamp>  directory entry
            VSSI_OpenDir2(handle, orphanedEntriesDirNameFullPath, &dir,
                          &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc = test_context.rv;
            if(rc != VSSI_SUCCESS) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                   "VSSI_OpenDir2 %s returns %d.", orphanedEntriesDirNameFullPath, rc);
                rv++;
                goto exit;
            }
            orphanedEntriesCount = 0;
            while ( dir && (dirent = VSSI_ReadDir2(dir)) != NULL ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s using VSSI_ReadDir2.", dirent->name);
                    orphanedEntriesCount++;
                    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s found using VSSI_ReadDir2.", dirent->name);
                    if(dirent->isDir == true) {
                        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is a directory", dirent->name);
                    } else {
                        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%s is a file", dirent->name);
                    }
            } 
                    
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d orphaned entries found in %s.", orphanedEntriesCount, orphanedEntriesDirNameFullPath);
            if (orphanedEntriesCount != TEST_DB_ORPHANED_ENTRIES ) {
                VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d orphaned entries found in %s.i Expected %d", orphanedEntriesCount, orphanedEntriesDirNameFullPath, TEST_DB_ORPHANED_ENTRIES);
                rv++;
            }
        }
        // OpenDir2 has succeeded so need to close dir
        ...
        orphanedEntriesCount = 0;
        foundOrphanedDirEntry = false;
        // check for missing parent entries - expected 5
        // Open orphaned-entries-<timestamp>  directory
        VSSI_OpenDir2(handle, orphanedEntriesNameFullPath, &dir,
                      &test_context, vscs_test_callback);
        VPLSem_Wait(&(test_context.sem));
        ...
        if (foundOrphanedDirEntry == false) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "second orphaned entry directory NOT found using VSSI_ReadDir2.");
            rv++;
            goto exit;
        }
        ...
        VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d orphaned entries found in %s.", orphanedEntriesCount, orphanedEntriesDirNameFullPath);
        if (orphanedEntriesCount != TEST_DB_ORPHANED_ENTRIES_2 ) {
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "%d orphaned entries found in %s.i Expected %d", orphanedEntriesCount, orphanedEntriesDirNameFullPath, TEST_DB_ORPHANED_ENTRIES);
            rv++;
        }
        // dont exit and do the 4 error test 
    }

    // 1. Check the files and/or directories

    // 1(a). Check for expected status when attempt to open
    //       and find directories/files via readDir
    ...
    // 1(b). Check for expected status of open file
    if ( db_err_hand_param == ReadFileExpectSuccess ||
         db_err_hand_param == ReadFileExpectNotFound ||
         db_err_hand_param == DB4Errors ||
         db_err_hand_param == CheckLostFoundOrphaned ||
        ...
        if ( (db_err_hand_param == DB4Errors) || (db_err_hand_param == CheckLostFoundOrphaned) ) {
            VSSI_OpenFile(handle, nameMissing, flags, attrs, &fileHandle2,
                          &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc2 = test_context.rv;

            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0,
                        "db_err_hand_param is %d    VSSI_OpenFile for missing file returned %d.",
                        db_err_hand_param, rc2);

            VSSI_OpenFile(handle, nameMissingFile, flags, attrs, &fileHandle0,
                          &test_context, vscs_test_callback);
            VPLSem_Wait(&(test_context.sem));
            rc0 = test_context.rv;
            ...
            VPLTRACE_LOG_ALWAYS(TRACE_APP, 0,
                        "db_err_hand_param is %d    VSSI_OpenFile for missing file returned %d.",
                        db_err_hand_param, rc0);
        }

        if ( db_err_hand_param == ReadFileExpectCCDStopped ||
             db_err_hand_param == ExpectDBCorruptionOnfsck) {
            // If ccd is finishes shutdown before we try open, we will get VSSI_COMM
            // Usually the CCD will not be stopped before we try open.
            if (rc != VSSI_COMM && rc != VSSI_NOTFOUND) {
                ...
                rc = time_to_traverse(test_context, handle, sdb_dirname_base, num_sdb_top_level_trees,
                                      num_sdb_subtrees, num_sdb_subtree_levels, num_sdb_subtree_subdirs,
                                      num_sdb_subtree_files, sdb_fileBaseSize, false);
                if (rc != VSSI_COMM && rc != DBEH_TTT_TREE_NOTFOUND) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                         "time_to_traverse for %s expected VSSI_COMM or VSSI_NOTFOUND, returns %d.",
                         sdb_dirname_base, rc);
                    rv++;
                }
            }
        ...
        else if ( (db_err_hand_param == DB4Errors) || (db_err_hand_param == CheckLostFoundOrphaned) ) {
            if (rc2 != VSSI_NOTFOUND) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
            ...
            // This is file with only component entry 
            // No content entry
            // No file
            VSSI_OpenFile(handle, zeroSizeFileName, flags, attrs, &fileHandle2,
                          &test_context, vscs_test_callback);
            ...
            else if ( db_err_hand_param == ReadDir2AndFile2ExpectFileNotFound ) {
                rc = time_to_traverse(test_context, handle, ldb_dirname_base, num_ldb_top_level_trees,
                                  num_ldb_subtrees, num_ldb_subtree_levels, num_ldb_subtree_subdirs,
                                  num_ldb_subtree_files, ldb_fileBaseSize, false);
                if (rc != DBEH_TDT_FILE_NOTFOUND) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                                     "time_to_traverse for %s expected DBEH_TDT_FILE_NOTFOUND, returns %d.",
                                     ldb_dirname_base, rc);
                    rv++;
                }
            }
            ...
            if (rc != VSSI_BADPATH) {
                ...
                rc = time_to_traverse(test_context, handle, ldb_dirname_base, num_ldb_top_level_trees,
                                  num_ldb_subtrees, num_ldb_subtree_levels, num_ldb_subtree_subdirs,
                                  num_ldb_subtree_files, ldb_fileBaseSize, false);
                if (rc != DBEH_TTT_TREE_NOTFOUND) {
                    VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                                     "time_to_traverse for %s expected DBEH_TTT_TREE_NOTFOUND, returns %d.",
                                     ldb_dirname_base, rc);
                    rv++;
                }
            }
...
//                             "%s", dirname);
        ...
        rc = time_to_traverse(test_context, handle, dirname_base, num_top_level_trees, num_subtrees, num_subtree_levels, num_subtree_subdirs, num_subtree_files, frag_fileBaseSize, true);
        if (rc != 0) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
        ...
        // 1(c).  Check file size and content via stat and read
        ...
        if (!rv && (
            db_err_hand_param == ReadFileExpectSuccess ||
            db_err_hand_param == CreateFileExpectSuccess ||
            db_err_hand_param == ReadFileExpectSuccessButFileLarger ||
            db_err_hand_param == ReadFileExpectSuccessButFileSmaller)) {
            ...
            if (rc != 0) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                    "Error traversing %s directory tree.", sdb_dirname_base);
                rv++;
            }
        }
        ...
        if (!rv && (
            ...
            if (rc != 0) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                    "Error traversing %s directory tree.", ldb_dirname_base);
                rv++;
            }
        }
    }
                             ...
                             bool longTest)
{
    ...
    VSSI_OpenObjectTS(location.c_str(),
...
//    rv += test_personal_cloud(user_id, dataset_id, run_cloudnode_tests);
    rv += test_personal_cloud(user_id, dataset_id, run_cloudnode_tests);
    if(rv) goto exit;

#ifdef NEEDS_CONVERSION
    rv += test_vscs_access_save_bvd_async(user_id, dataset_id);
    if(rv) goto exit;
#endif // NEEDS_CONVERSION

    rv += test_vss_poll_thread(user_id, dataset_id);
    if(rv) goto exit;

    rv += test_async_notification_events(user_id,
                                         dataset_id,  
                                         run_async_client_object_test);
    if(rv) goto exit;

    rv += test_vss_file_handles(user_id, dataset_id, false);
    if(rv) goto exit;

    rv += test_vss_file_handles(user_id, dataset_id, true);
    if(rv) goto exit;

    rv += test_vss_file_rename_remove(user_id, dataset_id);
    if(rv) goto exit;
    
    rv += test_vss_file_remove_shared(user_id,
                                      dataset_id);
    if(rv) goto exit;

    rv += test_vss_file_create(user_id,
                               dataset_id);
    if(rv) goto exit;

    rv += test_vss_file_rename_attributes(user_id,
                               dataset_id);
    if(rv) goto exit;

    rv += test_vss_file_modify_time(user_id,
                               dataset_id);
    if(rv) goto exit;

    rv += test_vss_file_attributes(user_id,
                                   dataset_id);

    rv += test_vss_mkdir_parents_and_existing(user_id,
                                              dataset_id);
    if(rv) goto exit;

    if (run_case_insensitivity_test) {
        rv += test_vss_file_case_insensitivity(user_id,
                                               dataset_id);
        if(rv) goto exit;
    }

    if (run_file_access_and_share_modes_test) {
        rv += test_vss_file_access_and_share_modes(user_id,
                                                   dataset_id);
        if(rv) goto exit;
    }

    if (run_many_file_open_test) {
        rv += test_vss_file_open_many(user_id, dataset_id);
        if(rv) goto exit;
    }

    rv += test_vss_file_oplocks(user_id,
                                dataset_id);
    if(rv) goto exit;

    rv += test_vss_file_oplocks_brlocks(user_id,
                                dataset_id);
    if(rv) goto exit;

    rv += test_vss_object_teardown(user_id,
                                dataset_id);
    if(rv) goto exit;

    rv += test_network_down(user_id,
                            dataset_id);
    if(rv) goto exit;

#ifdef TEST_BAD_OBJHANDLE
    // Manual test cases (require user intervention at server)
    rv += test_vss_bad_objhandle(user_id,
                                 dataset_id);
#endif
