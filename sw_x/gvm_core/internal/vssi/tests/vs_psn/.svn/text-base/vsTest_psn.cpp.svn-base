...
static void usage(int argc, char* argv[])
{
    ...
    printf(" -e --run-db-err-hand-test PARAM  Run test for db error handling, backup, or fsck test with integer parameter (%d).\n",
           db_err_hand_param);
    printf("                                  0 don't run the test, 1 expect create file 1 success,\n");
    printf("                                  2 expect read file 1 success, 3 expect file 1 not found,\n");
    printf("                                  4 expect db corruption, 5 expect db irrecoverably failed,\n");
    printf("                                  6 expect create file 2 success, 7 expect dir2 and read file 2 successs,\n");
    printf("                                  8 expect dir2 and file2 not found, 9 expect dir2 but file 2 not found,\n");
...
static void strip_route_info(VSSI_RouteInfo& route_info)
{
    // strip all but proxy routes from the route list
    // to force this thing into proxy mode.
    for( int i = 0 ; i < route_info.num_routes ; i++ ) {
        int left;

        if ( route_info.routes[i].type == VSSI_ROUTE_PROXY ) {
            continue;
        }
        ...
        left = route_info.num_routes - i;
        if ( left ) {
            memcpy(&route_info.routes[i], &route_info.routes[i+1],
                left * sizeof(VSSI_Route));
        }
        i--;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if (setjmp(vsTestErrExit) == 0) { // initial invocation of setjmp
        ...
        for(dataset_it = ownedDatasets.begin();
            ...
            handle = storage_it->accesshandle();
            if ( !storage_it->has_devspecaccessticket() ) {
                VPLTRACE_LOG_ERR(TRACE_BVS, 0,
                                    "storage has no dev spec ticket?");

            }
            ticket = storage_it->devspecaccessticket();
            rc = setup_route_access_info(*storage_it, *dataset_it,
            ...
            // Register session with the dev specific ticket
            vssi_session = VSSI_RegisterSession(handle, ticket.c_str());
            if(vssi_session == 0) {
                VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL: VSSI_RegisterSession()");
                rv++;
                goto exit;
            }
            ...
            if (db_err_hand_param) {
                VPLTRACE_LOG_ALWAYS(TRACE_BVS, 0,
                                    "Test database error handling %d db-dir %s with dataset %s, ID "FMTx64" with location \n\t%s.",
                                    db_err_hand_param,
                ...
                rc = test_db_error_handling(location.c_str(), user_id,
                                                  dataset_id, route_info, db_err_hand_param, db_dir);
                if(rc != 0) {
                    rv++;
                    VPLTRACE_LOG_ALWAYS(TRACE_BVS, 0,
            ...
            rc = test_vss_session_recognition(handle, ticket, location.c_str(),
        ...
        direct_skipped:
            ...
            if(route_info.num_routes > 0) {
                ...
                rc = test_vssi_dataset_access(location, user_id, dataset_id,
                                              ...
                                              run_cloudnode_tests,
                                              run_async_client_object_test,
                                              false, // disable for proxy run_file_access_and_share_modes_test
                                              run_many_file_open_test,
                                              run_case_insensitivity_test);
                VPLTRACE_LOG_ALWAYS(TRACE_BVS, 0,
                ...
                rc = test_vss_session_recognition(handle, ticket, location.c_str(),
                                                  user_id, dataset_id, route_info, false);
                VPLTRACE_LOG_ALWAYS(TRACE_BVS, 0,
                                    "TC_RESULT = %s ;;; TC_NAME = Session_Recognition_Proxy2_"FMTx64,
                                    rc ? "FAIL" : "PASS",
                                    dataset_it->datasetid());
                rv += rc;
                if(rv) goto exit;
