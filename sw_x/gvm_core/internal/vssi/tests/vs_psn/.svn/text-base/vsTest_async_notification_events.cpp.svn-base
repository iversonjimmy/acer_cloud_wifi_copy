...
int test_async_notification_events(VSSI_Session session,
                                   ...
                                   bool run_async_client_object_test)
{
    ...
    VPLTRACE_LOG_ALWAYS(TRACE_APP, 0, "Wait even queue to be cleaned for %d secs", EVENT_TIMEOUT);
    ...
    if(run_async_client_object_test) {
        ...
        if(rc == VSSI_SUCCESS) {
            VPLTRACE_LOG_ERR(TRACE_APP, 0, "FAIL:"
                             "Close object %s failed: %d expect failed.",
                             save_description, rc);
            rv++;
            goto fail;
        }
