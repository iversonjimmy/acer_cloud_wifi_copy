...
static void clear_dataset_callback(void* ctx, VSSI_Result rv)
{
...
int cloud_clear_dataset(VSSI_Session session, std::string& datasetLocation)
{
    ...
    VSSI_Delete(session, datasetLocation.c_str(),
                &test_context, clear_dataset_callback);
...
void cloud_done(void* ctx)
{
    ...
    VPLTRACE_LOG_INFO(TRACE_APP, 0,
                      ...
                      task->taskName.c_str());
    VSSI_CloseObject(task->handle, ctx, cloud_close_done_helper);
}
...
void cloud_write_block(void* ctx)
{
    ...
    VSSI_Write(task->handle, ioTask->filename.c_str(),
...
void cloud_write_size(void* ctx)
{
    ...
    VSSI_Truncate(ioTask->main_task->handle, ioTask->filename.c_str(),
                  ioTask->filesize,
...
void cloud_read_block(void* ctx)
{
    ...
    blockTask->start_time = VPLTime_GetTime();
    VSSI_Read(task->handle, ioTask->filename.c_str(),
              offset, &blockTask->req_len,
              blockTask->buffer,
