...
public:
    vs_trash(vs_dir* dir, 
             ...
             const std::string& name); // trashed a directory
    vs_trash(vs_file* file,
             VSSI_TrashRecord& record,
             const std::string& name); // trashed a file
    ~vs_trash();
    ...
    bool name_match(std::string& name)
    {
        return (record_name.compare(name) == 0);
    };
    ...
    void initialize(vs_file* file, vs_dir* dir,
                    VSSI_TrashRecord& record,
                    const std::string& name);
};
...
public:
    ...
    // Verify trashcan contents and server contents agree.
    int verify(VSSI_Object handle);

    // Empty the trashcan. Delete all records and take action with server.
    int empty_trashcan(VSSI_Object handle);
    
    // Delete a trash record and take action with the server.
    // Will restore the named record (name of thing deleted).
    int delete_record(VSSI_Object handle,
                      const std::string& name);
    
    // Restore a trash record back to the dataset.
    // If name is provided, rename the object. 
    // Otherwise, use the object's original name.
    // Will trash anything in the way of the recovered data.
    int restore_record(VSSI_Object handle,
                       const std::string& name,
                       const char* new_name);
    ...
    // Contained records;
    std::map<std::pair<u64, u32>, vs_trash*> records; // ID to record
};
