...
static int parse_args(int argc, char* argv[])
{
    ...
    for(;;) {
        ...
        int c = getopt_long(argc, argv, "vtu:p:d:q:ZWPoVi:r:D:F:L:CR:OI:c:S:", 
...
#ifdef NOTDEF
VPLTRACE_LOG_INFO(TRACE_APP, 0, "server %s - type %d", route_info.routes[route_ind].server, route_info.routes[route_ind].type);
        if ( route_info.routes[route_ind].type == VSSI_ROUTE_DIRECT_EXTERNAL) {
            VPLTRACE_LOG_INFO(TRACE_APP, 0, "Converting external route to internal - server %s", route_info.routes[route_ind].server);
            route_info.routes[route_ind].type = VSSI_ROUTE_DIRECT_INTERNAL;
        }
#endif // NOTDEF
