...
static u16 util_unicode_upcase_table[] = {
    0x0000,
    0x0001,
    0x0002,
    0x0003,
...
#define UNICODE_UPPER(x)   (((x) < 0x2500) ? util_unicode_upcase_table[(x)] : (((x) >= 0xff41) && ((x) < 0xff5b)) ? ((x) - 0x20) : (x))
...
void Util_UTF8Upper(const char *s, std::string &s_upper)
{
    std::string c_upper;
    int i = 0, skip, code;
...
std::string Util_UTF8Upper(const char *s)
{
    std::string result;
    Util_UTF8Upper(s, result);
    return result;
}

int Util_UTF8CaseNCmp(int s1_len, const char *s1, int s2_len, const char *s2)
{
    int i = 0;
    ...
    while (i < min_len) {
        int skip_1;
        int skip_2;
        int code_1 = UNICODE_UPPER(get_next_code(&s1[i], &skip_1));
        int code_2 = UNICODE_UPPER(get_next_code(&s2[i], &skip_2));
