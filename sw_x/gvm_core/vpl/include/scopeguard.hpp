...
/// Create an anonymous ScopeGuard to call the specified function at the end of the current scope.
/// @example
///   {
///       void* myPtr = malloc(x);
///       if (myPtr == NULL) {
///           return -1;
///       }
///       ON_BLOCK_EXIT(free, myPtr);
///       ...
///   }
#define ON_BLOCK_EXIT ScopeGuard ANONYMOUS_VARIABLE(scopeGuard) ATTRIBUTE_UNUSED = MakeGuard

/// Create an anonymous ScopeGuard to call the specified object's specified method at the end
/// of the current scope.
/// @example
///   ON_BLOCK_EXIT_OBJ(*this, &className::methodName);
#define ON_BLOCK_EXIT_OBJ ScopeGuard ANONYMOUS_VARIABLE(scopeGuard) ATTRIBUTE_UNUSED = MakeObjGuard

/// For use with #ON_BLOCK_EXIT.
/// @example
///   u64* deviceIds = new u64[numDevices];
///   ON_BLOCK_EXIT(deleteArray<u64>, deviceIds);
/// @note boost::scoped_array may be cleaner if that is available.
template<class T>
...
/// For use with #ON_BLOCK_EXIT.
/// @note If you are using this, you may want to alternatively consider using std::autoptr
///     instead of #ON_BLOCK_EXIT.
template<class T>
static inline void deleteObj(T* p)
{
    delete p;
}
