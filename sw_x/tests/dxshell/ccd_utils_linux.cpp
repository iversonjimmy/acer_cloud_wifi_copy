...
static int startCcdWithPath(const char* ccdPath, const char* titleId)
{
    int rv = 0;
    std::string tmpPath;
	LOG_ALWAYS("enter startCcdWithPath");
	//LOG_ALWAYS("tmpPath: %s\n", &tmpPath);
    rv = checkRunningCcd();
	LOG_ALWAYS("after checkRunningCcd");
	//std::out << "test!!!!!!" << std::endl;
	//std::out << "rv: " << rv << std:endl;
    if (rv != 0) {
...
{
    LOG_ALWAYS("before startCcdWithPath");
	//using namespace std;
	//std::out << "titleId: " << titleId << std::endl;
    return startCcdWithPath("./ccd", titleId);
    LOG_ALWAYS("after startCcdWithPath");
}
