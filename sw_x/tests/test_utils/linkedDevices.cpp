...
static void print_usage_and_exit(const char *progname)
{
    std::cerr
        ...
        << "\t[-d id[,id,...]]\tunlink specified device IDs (default is none)" << std::endl
        << "\t[-D]\t\t\tunlink all devices (default it not)" << std::endl
...
public:
    ...
    std::string username;
    std::string password;
    std::vector<u64> deviceIdsToUnlink;
    bool unlinkAllDevices;
    std::string iashost;
    u16 iasport;
    std::string vsdshost;
    u16 vsdsport;
        ...
        if (argv[i][0] == '-') {
            ...
            case 'd':
                if (i+1 < argc) {
                    ...
                    while (p != NULL) {
                        deviceIdsToUnlink.push_back(VPLConv_strToU64(p, NULL, 10));
                        p++;  // skip ','
                        p = strtok(NULL, ",");
                    }
                }
...
int main(int argc, char *argv[])
{
    ...
    vplex::vsDirectory::GetLinkedDevicesInput req;
    vplex::vsDirectory::GetLinkedDevicesOutput res;
    *(req.mutable_session()) = session;
    req.set_userid(uid);
    rc = VPLVsDirectory_GetLinkedDevices(proxy, 30, req, res);
    if (rc != 0) {
        LOG_ERROR("VPLVsDirectory_GetLinkedDevices failed: %d", rc);
        exit(1);
    }

    std::cout << "Found " << res.devices_size() << " device(s)" << std::endl;

    google::protobuf::RepeatedPtrField<vplex::vsDirectory::DeviceInfo>::const_iterator it;
    for (it = res.devices().begin(); it != res.devices().end(); it++) {
        std::cout << std::dec << it->deviceid() << " (0x" << std::hex << it->deviceid() << ")  " << it->devicename() << std::endl;
        if (args.unlinkAllDevices) {
            args.deviceIdsToUnlink.push_back(it->deviceid());
        }
    }

    if (!args.deviceIdsToUnlink.empty()) {
        std::vector<u64>::const_iterator it;
        for (it = args.deviceIdsToUnlink.begin(); it != args.deviceIdsToUnlink.end(); it++) {
            vplex::vsDirectory::UnlinkDeviceInput req2;
            vplex::vsDirectory::UnlinkDeviceOutput res2;
            *(req2.mutable_session()) = session;
            req2.set_userid(uid);
            req2.set_deviceid(*it);
            LOG_INFO("Unlinking deviceId "FMTu64"...", *it);
            int rc = VPLVsDirectory_UnlinkDevice(proxy, 30, req2, res2);
            if (rc != 0) {
                LOG_ERROR("VPLVsDirectory_UnlinkDevice failed on ("FMT_VPLUser_Id_t","FMTu64"): %d",
                          uid, *it, rc);
            }
        }
    }
    ...
    {
        vplex::vsDirectory::GetDatasetDetailsInput req;
        vplex::vsDirectory::GetDatasetDetailsOutput res;
        *(req.mutable_session()) = session;
        req.set_userid(uid);
        req.set_datasetid(8367769);
        rc = VPLVsDirectory_GetDatasetDetails(proxy, 30, req, res);
