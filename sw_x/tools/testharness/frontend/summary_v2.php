...
# get test run info
$query = "SELECT * FROM test_run ORDER BY id DESC LIMIT 1000";
...
while ( $i < $num_rows ) {
    ...
    $tc_expected_to_fail = mysql_result($result,$i,"tc_expected_to_fail");
...
#    $tc_remaining = mysql_result($result,$i,"tc_remaining");
    ...
    if ( $run_status == "PASS" || $run_staus == "EXPECTED_TO_FAIL" ) {
    ...
    if ( $output_dir == "" ) {
        ...
        if ( $pos === false ) {
            ...
            $html_txt = str_replace("build@pcstore.ctbg.acer.com:/a/","",$output_dir);
            $html_txt = str_replace("build@pcstore.ctbg.acer.com:a/","",$html_txt);
            $output_txt = "<a href=\"http://pcstore.ctbg.acer.com/$html_txt\">$output_dir</a>";
        }
    }
    ...
    echo "        <td><a href=\"show_test_runs_v2.php?runID=$run_id\">$run_id</a></td>\n";
    echo "        <td><a href=\"show_test_runs_v2.php?runID=$run_id\">$run_status</a></td>\n";
    echo "        <td>$run_name</td>\n";
    ...
    echo "        <td>$tc_pass</td>\n";
    echo "        <td>$tc_fail</td>\n";
    echo "        <td>$tc_expected_to_fail</td>\n";
#    echo "        <td>$tc_skip</td>\n";
