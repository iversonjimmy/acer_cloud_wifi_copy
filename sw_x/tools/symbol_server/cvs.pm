...
sub SourceStreamVariables {
    ...
    if ( defined $$self{CVS_DATE} && $$self{CVS_DATE} ne "" && defined $$self{CVS_LABEL} && $$self{CVS_LABEL} ne "" ) {
        ...
        push(@stream, "CVS_EXTRACT_CMD=%fnchdir%(%CVS_WORKINGDIR%)".
                                       ...
                                       "export ".
                                       "-n -r %cvslabeltag% -D \"%cvsdate%\" ".
                                       "-d %cvslabeltag%_%cvsdatetarg% " .
                                       "%var3%");
        push(@stream, "CVS_EXTRACT_TARGET=".
                      "%targ%\\%var2%\\%fnbksl%(%var3%)\\%CVSLABELTAG%_%cvsdatetarg%".
                      "\\%fnfile%(%var1%)");
    
    } elsif ( defined $$self{CVS_DATE} && $$self{CVS_DATE} ne "" ) {
